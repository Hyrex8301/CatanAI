...
- **M4 (search bots, brief `docs/M4-brief.md`, approved 2026-09-23): step 1 done**: the trainer's champion challenge has three stages (`Trainer.Challenge`: screen 480 games at 95%, confirm 960 more at 99% over all, no regression: ≥25% against the start + pool); `Catan.Sim ladder --bots a,b,c` plays every pair 2v2 and prints pairwise rates and Bradley-Terry ratings (`Ladder`). First ladder: trained `best.json` beats default-weight SmartBot 61.5% ± 4.8% in 2v2 (about +80 rating). Thinking time will be picked by the user from ladder numbers at checkpoint B.
