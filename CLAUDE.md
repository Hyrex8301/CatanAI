...
- **M3 (smart bots + overnight training): steps 1–6 done, checkpoints A and B passed; at checkpoint C** (first overnight run, started by the user 2026-09-22). SmartBot (Play settings) beat 3 RandomBots in 400/400 games; `Catan.Sim match` compares bots (identical bots split 25.4% ± 1.3%, so it's fair). A 3-minute trainer check crowned 4 champions; the latest won 39.6% vs the starting weights (fair = 25%).
